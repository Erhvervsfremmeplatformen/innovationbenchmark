                      ...
                      <div v-if="currentSection === 'test1'" class="col-md-6">
                        <div :class="['card', 'card-transparent']">
                          <div class="card-header">
                            <h2>
                              <!-- @vue-ignore -->
                              {{ frontPageMatter.cards[frontPageMatter.cards.length - 2].cardHeadline }}
                            </h2>
                          </div>
                          <!-- @vue-ignore -->
...
export default defineComponent({
  ...
  data() {
    return {
      ...
      currentStep: 1 as number, // initial 1
      ...
      test1: null as Test1 | null,
      test2: null as Test2 | null,
      frontPageMatter: null as FrontPageMatter | null,
      results1: null as Results1 | null,
      results2: null as Results2 | null,
      initialValues: {
        virk_navn: '',
        industri: 0,
        navn: '',
  ...
  methods: {
    ...
    calculateSliders(name: string, values: any, fields: SliderField[], setValue: any) {
      const sliders = fields.filter(field => field._type === 'slider');
      const sum = sliders.reduce((accumulator, field: any) => accumulator + (Number(values[field.key as string]) - 1), 0);
      if (sum > 10) {
